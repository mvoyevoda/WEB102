// import React, { useState, useEffect } from "react";
// import { createClient } from "@supabase/supabase-js";
// import { BrowserRouter as Router, Route, Link, Routes } from "react-router-dom";
// import CrewmateDetails from "./components/CrewmateDetails.jsx";
// import "./App.css";
// import Form from "./components/form.jsx";

// const supabaseUrl = "https://opcsqcaivuonyyhiqyso.supabase.co";
// const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9wY3NxY2FpdnVvbnl5aGlxeXNvIiwicm9sZSI6ImFub24iLCJpYXQiOjE2ODEzNDI3MzgsImV4cCI6MTk5NjkxODczOH0.6X-1oPrvUfSxpz1E0k6JPrUDcSbVqdY7OYzFK0sCi6k";

// const supabase = createClient(supabaseUrl, supabaseKey);

// function App() {
//   const [crewmates, setCrewmates] = useState([]);
//   const [name, setName] = useState("");
//   const [favorite, setFavorite] = useState("");
//   const [selectedCrewmate, setSelectedCrewmate] = useState(null);

//   useEffect(() => {
//     fetchCrewmates();
//   }, []);

//   async function showCrewmateDetails(id) {
//     const selected = crewmates.find((crewmate) => crewmate.id === id);
//     setSelectedCrewmate(selected);
//     window.location = `/crewmate/${id}`;
//   }

//   async function fetchCrewmates() {
//     const { data: crewmates } = await supabase.from("crewmates").select("*");
//     setCrewmates(crewmates);
//   }

//   async function addCrewmate() {
//     if (name && favorite) {
//       await supabase.from("crewmates").insert([{ name, favorite }]);
//       fetchCrewmates();
//       setName("");
//       setFavorite("");
//     }
//   }

//   async function deleteCrewmate(id) {
//     await supabase.from("crewmates").delete().match({ id });
//     fetchCrewmates();
//   }

//   async function updateCrewmate(id, name, favorite) {
//     if (id && name && favorite) {
//       await supabase
//         .from("crewmates")
//         .update({ name, favorite })
//         .match({ id });
//       setSelectedCrewmate(null);
//       fetchCrewmates();
//     }
//   }
  

//   return (
//     <Router>
//       <div className="App">
//         <div className="top">
//           <h1>Crewmates</h1>
//           <Routes>
//             <Route path="/" element={
//               <>
//                 <Form />
//                 <ul>
//                   {crewmates.map((crewmate) => (
//                     <li key={crewmate.id}>
//                       <img
//                         src="./src/assets/default.png"
//                         alt="Crewmate"
//                         onClick={() => showCrewmateDetails(crewmate.id)}
//                       />
//                       <span>{crewmate.name}</span>
//                     </li>
//                   ))}
//                 </ul>
//               </>
//             } />
//             <Route
//               path="/crewmate/:id"
//               element={<CrewmateDetails crewmate={selectedCrewmate} deleteCrewmate={deleteCrewmate} updateCrewmate={updateCrewmate} />}
//             />
//           </Routes>
//         </div>
//       </div>
//     </Router>
//   );
// }

// export default App;






// import React, { useState, useEffect } from "react";
// import { createClient } from "@supabase/supabase-js";
// import { BrowserRouter as Router, Route, Link, Routes } from "react-router-dom";
// import CrewmateDetails from "./components/CrewmateDetails.jsx";
// import "./App.css";
// import Form from "./components/form.jsx";

// const supabaseUrl = "https://opcsqcaivuonyyhiqyso.supabase.co";
// const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9wY3NxY2FpdnVvbnl5aGlxeXNvIiwicm9sZSI6ImFub24iLCJpYXQiOjE2ODEzNDI3MzgsImV4cCI6MTk5NjkxODczOH0.6X-1oPrvUfSxpz1E0k6JPrUDcSbVqdY7OYzFK0sCi6k";

// const supabase = createClient(supabaseUrl, supabaseKey);

// function App() {
//   const [crewmates, setCrewmates] = useState([]);
//   const [name, setName] = useState("");
//   const [favorite, setFavorite] = useState("");
//   const [selectedCrewmate, setSelectedCrewmate] = useState(null);

//   useEffect(() => {
//     fetchCrewmates();
//   }, []);

//   async function showCrewmateDetails(id) {
//     const selected = crewmates.find((crewmate) => crewmate.id === id);
//     setSelectedCrewmate(selected);
//     window.location = `/crewmate/${id}`;
//   }

//   async function fetchCrewmates() {
//     const { data: crewmates } = await supabase.from("crewmates").select("*");
//     setCrewmates(crewmates);
//   }

//   async function addCrewmate() {
//     if (name && favorite) {
//       await supabase.from("crewmates").insert([{ name, favorite }]);
//       fetchCrewmates();
//       setName("");
//       setFavorite("");
//     }
//   }

//   async function deleteCrewmate(id) {
//     await supabase.from("crewmates").delete().match({ id });
//     fetchCrewmates();
//   }

//   async function updateCrewmate(id, name, favorite) {
//     if (id && name && favorite) {
//       await supabase
//         .from("crewmates")
//         .update({ name, favorite })
//         .match({ id });
//       setSelectedCrewmate(null);
//       fetchCrewmates();
//     }
//   }

//   return (
//     <Router>
//       <div className="App">
//         <div className="top">
//           <h1>Crewmates</h1>
//           <Routes>
//             <Route path="/" element={
//               <>
//                 <Form 
//                   name={name} 
//                   setName={setName} 
//                   favorite={favorite} 
//                   setFavorite={setFavorite} 
//                   addCrewmate={addCrewmate} 
//                 />
//                 <ul>
//                   {crewmates.map((crewmate) => (
//                     <li key={crewmate.id}>
//                       <img
//                         src="./src/assets/default.png"
//                         alt="Crewmate"
//                         onClick={() => showCrewmateDetails(crewmate.id)}
//                       />
//                       <span>{crewmate.name}</span>
//                     </li>
//                   ))}
//                 </ul>
//               </>
//             } />
//             <Route
//               path="/crewmate/:id"
//               element={<CrewmateDetails crewmate={selectedCrewmate} deleteCrewmate={deleteCrewmate} updateCrewmate={updateCrewmate} />}
//             />
//           </Routes>
//         </div>
//       </div>
//     </Router>
//   );
// }

// export default App;




// import React, { useState, useEffect } from "react";
// import { createClient } from "@supabase/supabase-js";
// import { BrowserRouter as Router, Route, Link, Routes } from "react-router-dom";
// import CrewmateDetails from "./components/CrewmateDetails.jsx";
// import "./App.css";
// import Form from "./components/form.jsx";
// // import CrewmatesList from "./components/CrewmatesList.jsx";

// const supabaseUrl = "https://opcsqcaivuonyyhiqyso.supabase.co";
// const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9wY3NxY2FpdnVvbnl5aGlxeXNvIiwicm9sZSI6ImFub24iLCJpYXQiOjE2ODEzNDI3MzgsImV4cCI6MTk5NjkxODczOH0.6X-1oPrvUfSxpz1E0k6JPrUDcSbVqdY7OYzFK0sCi6k";

// const supabase = createClient(supabaseUrl, supabaseKey);

// function App() {
//   const [crewmates, setCrewmates] = useState([]);
//   const [name, setName] = useState("");
//   const [favorite, setFavorite] = useState("");
//   const [selectedCrewmate, setSelectedCrewmate] = useState(null);

//   useEffect(() => {
//     fetchCrewmates();
//   }, []);

//   async function showCrewmateDetails(id) {
//     const selected = crewmates.find((crewmate) => crewmate.id === id);
//     setSelectedCrewmate(selected);
//     window.location = `/crewmate/${id}`;
//   }

//   async function fetchCrewmates() {
//     const { data: crewmates } = await supabase.from("crewmates").select("*");
//     setCrewmates(crewmates);
//   }

//   async function addCrewmate() {
//     if (name && favorite) {
//       await supabase.from("crewmates").insert([{ name, favorite }]);
//       fetchCrewmates();
//       setName("");
//       setFavorite("");
//     }
//   }

//   async function deleteCrewmate(id) {
//     await supabase.from("crewmates").delete().match({ id });
//     fetchCrewmates();
//   }

//   async function updateCrewmate(id, name, favorite) {
//     if (id && name && favorite) {
//       await supabase
//         .from("crewmates")
//         .update({ name, favorite })
//         .match({ id });
//       setSelectedCrewmate(null);
//       fetchCrewmates();
//     }
//   }

//   return (
//     <Router>
//       <div className="App">
//         <div className="top">
//           <h1>Crewmates</h1>
//           <Routes>
//             <Route path="/" element={
//               <>
//                 <Form />
//                 <ul>
//                   {crewmates.map((crewmate) => (
//                     <li key={crewmate.id}>
//                       <img
//                         src="./src/assets/default.png"
//                         alt="Crewmate"
//                         onClick={() => showCrewmateDetails(crewmate.id)}
//                       />
//                       <span>{crewmate.name}</span>
//                     </li>
//                   ))}
//                 </ul>
//               </>
//             } />
//             <Route
//               path="/crewmate/:id"
//               element={<CrewmateDetails crewmate={selectedCrewmate} deleteCrewmate={deleteCrewmate} updateCrewmate={updateCrewmate} />}
//             />
//           </Routes>
//         </div>
//       </div>
//     </Router>
//   );
// }

// export default App;


import React, { useState, useEffect } from "react";
import { createClient } from "@supabase/supabase-js";
import { BrowserRouter as Router, Route, Link, Routes } from "react-router-dom";
import Details from "./components/Details";
import "./App.css";
import AddCrewmateForm from "./components/AddCrewmateForm";
import CrewmatesList from "./components/CrewmatesList";

const supabaseUrl = "https://opcsqcaivuonyyhiqyso.supabase.co";
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9wY3NxY2FpdnVvbnl5aGlxeXNvIiwicm9sZSI6ImFub24iLCJpYXQiOjE2ODEzNDI3MzgsImV4cCI6MTk5NjkxODczOH0.6X-1oPrvUfSxpz1E0k6JPrUDcSbVqdY7OYzFK0sCi6k";
const supabase = createClient(supabaseUrl, supabaseKey);

function App() {
  const [crewmates, setCrewmates] = useState([]);

  async function deleteCrewmate(id) {
    await supabase.from('crewmates').delete().eq('id', id);
  }

  useEffect(() => {
    fetchCrewmates();
  }, []);

  async function fetchCrewmates() {
    const { data: crewmates } = await supabase.from("crewmates").select("*");
    setCrewmates(crewmates);
  }

  return (
    <Router>
      <div className="App">
        <div className="top">
          <h1>
            <Link to="/">Crewmates</Link>
          </h1>
        </div>
        <Routes>
          <Route
            path="/"
            element={<CrewmatesList crewmates={crewmates} />}
            index
          />
          <Route
            path="/add"
            element={<AddCrewmateForm supabase={supabase} />}
          />
          <Route
            path="/crewmate/:id"
            element={<Details supabase={supabase} deleteCrewmate={deleteCrewmate} />}
          />
        </Routes>
      </div>
    </Router>
  );
}

export default App;
